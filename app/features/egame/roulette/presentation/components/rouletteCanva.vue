...
const msgColor = ref<string>('#94a3b8')
...
const spinWheel = async () => {
  ...
  // 2. Retrait immédiat de la mise dans le store local
  ...
  setTimeout(async () => {
    ...
    if (item.type === 'skull') {
      msgText.value = `💀 Dommage ! C'est une tête de mort. Vous perdez ${bet.toLocaleString('fr-FR')} XOF.`
      msgColor.value = "#ef4444"
      await fetchBalance()
    } else {
      ...
      triggerConfetti()
      showToast(`Gagné ! +${gains.toLocaleString('fr-FR')} XOF`, "fi-rr-check", "success")
      msgText.value = `🎉 Multiplicateur ${item.label} touché ! +${gains.toLocaleString('fr-FR')} XOF`
